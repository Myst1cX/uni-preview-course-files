// ==UserScript==
// @name         test.before.Preview PDFs, .DOCX, .PPTX and .XLSX files on E-Ucenje
...
// --- Function to add preview buttons to links ---
function addPreviewButtons() {
    document.querySelectorAll('a[href]').forEach(link => {
        const href = link.getAttribute('href');
        const fullUrl = resolveUrl(href);
        if (!fullUrl) return;
        if (link.closest('.preview-wrapper')) return;

        const ext = getFileExtension(fullUrl.toLowerCase());
        const isPreviewable = isEUcenje && (
    ...
    // Only resolve final URL for resource/forum links; skip assignment pluginfile links
    ...
    try {
        if (finalExt === 'pdf') await openPdfBlobViewer(targetUrl);
        else if (finalExt === 'docx') await openDocxWithMammoth(targetUrl);
        else if (finalExt === 'pptx') await openPptxWithPptxJs(targetUrl);
        else if (finalExt === 'xlsx') await openXlsxWithSheetJs(targetUrl);
        else if (['doc','ppt','xls'].includes(finalExt))
            alert(`Preview not supported for .${finalExt} files. Please convert to modern format.`);
        else alert('Preview not supported for this file type.');
    } catch (err) {
